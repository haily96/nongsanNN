...
class _GioHangScreen extends State<GioHangScreen>{
  ...
  Widget build(BuildContext context) {
    return WillPopScope(child: Scaffold(
      ...
      body:  Column(
        children: [
          listGioHang.isNotEmpty ? Expanded(child: ListView.builder(
              itemCount: listGioHang.length,
              itemBuilder: (context,index)=> itemGioHang(listGioHang[index]))) : Column(
            children: [
              Container(
                width: double.infinity,
                height: 50,
                alignment: Alignment.center,
                child: Text("Giỏ hàng trống !",style: GoogleFonts.oswald(
                  fontSize: 18,
                  color: Colors.grey
                ),),
              )
          ...
          listGioHang.isNotEmpty ? Container(
            width: 160,
            height: 50,
            ...
            child: ElevatedButton(
              ...
              child: Text("Đặt hàng",style: GoogleFonts.oswald(
                fontSize: 18,
        ...
        children: [
         ...
         Expanded(flex: 1,child: Column(
           ...
           children: [
             ...
             Padding(padding: const EdgeInsets.only(left: 5),child: Text("Số lượng: ${gioHangModel.soluong}"),),
             Padding(padding: const EdgeInsets.only(left: 5),child: Text("Tổng: ${gioHangModel.tongtien}",style: GoogleFonts.oswald(
                 fontSize: 17,
                 fontWeight: FontWeight.w700,
                 color: Colors.red
             ),),),
          ...
          IconButton(onPressed: (){
            setState(() {
              FirebaseFirestore.instance.collection("GioHang")
              .doc(gioHangModel.id.toString()).delete().then((value){
          ...
          child:  Column(
            children: [
              ...
              Container(
                ...
                child: Text("Tổng tiền :$tongtien",style: GoogleFonts.oswald(
                  fontSize: 18,
                  color: Colors.red,
                  fontWeight: FontWeight.w700
                ),),
                  ...
                  onPressed: ()async{
                    ...
                    if(hoten.isNotEmpty){
                      if(diachi.isNotEmpty){
                        int result = await DBManager.instances.addHoaDon(hoten, diachi, tongtien);
                        if(result>0){
                          ...
                          hash.putIfAbsent("tongtien", () => tongtien.toString());
                          ...
                          .add(hash).then((value) {
                             if(value.id.isNotEmpty){
                               for(GioHangModel sapham  in listGioHang){
                                 HashMap<String,String> hash = HashMap();
                                 hash.putIfAbsent("giatien", () => sapham.tongtien.toString());
                                 hash.putIfAbsent("tensp", () => sapham.name.toString());
